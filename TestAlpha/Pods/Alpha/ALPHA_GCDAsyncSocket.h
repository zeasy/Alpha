...
@class ALPHA_GCDAsyncReadPacket;
@class ALPHA_GCDAsyncWritePacket;
@class ALPHA_GCDAsyncSocketPreBuffer;
...
extern NSString *const ALPHA_GCDAsyncSocketException;
extern NSString *const ALPHA_GCDAsyncSocketErrorDomain;

extern NSString *const ALPHA_GCDAsyncSocketQueueName;
extern NSString *const ALPHA_GCDAsyncSocketThreadName;

#if SECURE_TRANSPORT_MAYBE_AVAILABLE
extern NSString *const ALPHA_GCDAsyncSocketSSLCipherSuites;
#if TARGET_OS_IPHONE
extern NSString *const ALPHA_GCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const ALPHA_GCDAsyncSocketSSLProtocolVersionMax;
#else
extern NSString *const ALPHA_GCDAsyncSocketSSLDiffieHellmanParameters;
#endif
#endif

enum ALPHA_GCDAsyncSocketError
{
	ALPHA_GCDAsyncSocketNoError = 0,           // Never used
	ALPHA_GCDAsyncSocketBadConfigError,        // Invalid configuration
	ALPHA_GCDAsyncSocketBadParamError,         // Invalid parameter was passed
	ALPHA_GCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	ALPHA_GCDAsyncSocketReadTimeoutError,      // A read operation timed out
	ALPHA_GCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	ALPHA_GCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	ALPHA_GCDAsyncSocketClosedError,           // The remote peer closed the connection
	ALPHA_GCDAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum ALPHA_GCDAsyncSocketError ALPHA_GCDAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface ALPHA_GCDAsyncSocket : NSObject

/**
 * ALPHA_GCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, ALPHA_GCDAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means ALPHA_GCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means ALPHA_GCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, ALPHA_GCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, ALPHA_GCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of ALPHA_GCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a ALPHA_GCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a ALPHA_GCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that ALPHA_GCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - ALPHA_GCDAsyncSocketSSLCipherSuites
 * - ALPHA_GCDAsyncSocketSSLProtocolVersionMin
 * - ALPHA_GCDAsyncSocketSSLProtocolVersionMax
 * 
 * If SecureTransport is available on Mac OS X:
 * 
 * - ALPHA_GCDAsyncSocketSSLCipherSuites
 * - ALPHA_GCDAsyncSocketSSLDiffieHellmanParameters;
 * 
 * 
...
/**
 * ALPHA_GCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the ALPHA_GCD API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(ALPHA_GCDAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(ALPHA_GCDAsyncSocket *)sock didAcceptNewSocket:(ALPHA_GCDAsyncSocket *)newSocket
 * {
 ...
 *   The faster and more powerful ALPHA_GCD API isn't notified properly in this case.
 * 
 ...
 * Example usage:
 * 
 * - (void)socket:(ALPHA_GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol ALPHA_GCDAsyncSocketDelegate
@optional
...
**/
- (void)socket:(ALPHA_GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(ALPHA_GCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(ALPHA_GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(ALPHA_GCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(ALPHA_GCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(ALPHA_GCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(ALPHA_GCDAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(ALPHA_GCDAsyncSocket *)sock withError:(NSError *)err;
...
**/
- (void)socketDidSecure:(ALPHA_GCDAsyncSocket *)sock;
