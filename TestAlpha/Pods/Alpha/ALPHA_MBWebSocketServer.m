...
@interface ALPHA_MBWebSocketServer () <ALPHA_GCDAsyncSocketDelegate>
@end

@interface NSString (ALPHA_MBWebSocketServer)
- (id)sha1base64;
@end

static unsigned long long ntohll2(unsigned long long v) {
...
@implementation ALPHA_MBWebSocketServer
@dynamic clientCount;

- (id)initWithPort:(NSUInteger)port delegate:(id<ALPHA_MBWebSocketServerDelegate>)delegate {
    _port = port;
    _delegate = delegate;
    socket = [[ALPHA_GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)];
    ...
    if (error) {
        NSLog(@"ALPHA_MBWebSockerServer failed to initialize: %@", error);
        return nil;
    }
...
- (BOOL)acceptOnPort:(uint16_t)port error:(NSError **)errPtr {
    ...
    socket = [[ALPHA_GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)];
    BOOL b=  [socket acceptOnPort:port error:errPtr] ;
    if(b) {
        _port = port;
    }
    return b;
...
- (void)send:(id)object {
    ...
    for (ALPHA_GCDAsyncSocket *connection in connections)
...
#pragma mark - ALPHA_GCDAsyncSocketDelegate

- (void)socket:(ALPHA_GCDAsyncSocket *)sock didAcceptNewSocket:(ALPHA_GCDAsyncSocket *)connection {
    [connections addObject:connection];
    [connection readDataWithTimeout:-1 tag:1];
}

- (void)socket:(ALPHA_GCDAsyncSocket *)connection didReadData:(NSData *)data withTag:(long)tag
{
    @try {
...
- (void)socket:(ALPHA_GCDAsyncSocket *)connection didWriteDataWithTag:(long)tag {
    if (tag == 2) {
...
- (void)socketDidDisconnect:(ALPHA_GCDAsyncSocket *)connection withError:(NSError *)error {
...
@implementation NSString (ALPHA_MBWebSocketServer)
...
@implementation NSData (ALPHA_MBWebSocketServer)
...
@implementation ALPHA_GCDAsyncSocket (ALPHA_MBWebSocketServer)
