...
- (float)alpha_cpuUsage {
    ...
    for (j = 0; j < thread_count; j++) {
        ...
        if(dispatch_queue_ptr && isMemoryReadable(id_info_th, sizeof(*id_info_th)))
        {
            dispatch_queue_t dispatch_queue = *dispatch_queue_ptr;
            const char * label = dispatch_queue_get_label((__bridge dispatch_queue_t)dispatch_queue);
        
            if (strcmp(label, "yzt.alpha.thread") == 0) {
                continue;
            }
        }
